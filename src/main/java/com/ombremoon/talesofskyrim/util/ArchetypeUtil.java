...
public class ArchetypeUtil {
    ...
    public static ISkyrimCapability get(Player player) {
        return player.getCapability(SkyrimCapabilityProvider.SKYRIM_CAPABILITY).orElseThrow(NullPointerException::new);
    }
    ...
    public static Map<AbstractSpell, SpellInstance> getActiveSpells(Player player) {
        return get(player).getActiveSpells();
    }
